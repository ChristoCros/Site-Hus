...
$compteArticle = executeRequete(" SELECT COUNT(id_article) AS nbArt FROM article ");
		...
		if(($_GET['p']-1) < 1 ){
			...
			for($i = 1; $i <= $nbPage; $i++){
				if($i == $cPage){
					...
					echo "<a href='shop.php?p=$i'>$i</a>";
				}
			}
		if(($_GET['p']+1) > $nbPage){
			echo "<a href='shop.php?p=$nbPage'>&gt;</a>";
		}
		else{
			echo "<a href='shop.php?p=" . ($_GET['p']+1) . "'>&gt;</a>";					
		}
		...
		echo'</div>';
			if($article['id_promo'] == 1){
				...
				echo'<div class="fond_prixPromo">';
					echo"<p><span class='old_price'>$article[prix]€</span> ". virgule(prixAvecPromo($article['id_article'])) . "€</p>";
				echo '</div>';						
			}
